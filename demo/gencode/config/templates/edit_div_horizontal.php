...
<?php endif;?>
		<div class="form-view">
			<form id="my_form" class="form-horizontal" method="post" action="{at_url('<?php echo $module_class_name;?>/edit', array('id'=>$data['id']))}">
			<?php
				$script_files = "";
				$scripts = "";
				$hiddens = "";
			...
			<div class="form-container">
				<?php 
					foreach ($fields as $v)
	                {
	                	...
	                	if( $field_setting['edit_status'] == 'hide' )
	                	{
	                		continue;
	                	}
	                	list($html, $script, $script_file, $display) = md_build_field($field_setting, 'edit', $module_class_name); 
	                	if( $display == 'hidden')
	                	{
	                		$hiddens .= $html;
	                		continue;
	            ...
	            <div class="control-group">
				    <label class="control-label">{lang('<?php echo $field_setting['field'];?>')}</label>
				    <div class="controls">
				    <?php echo $html; ?>
				    <?php if(at_array_val($field_setting, 'tips')):?>
						<p class="help-block"><i class="icon-info-sign"></i>{lang('tip_<?php echo $field_setting['field'];?>')}</p>
		            <?php endif; ?>
		        	</div>
		            <?php
...
$(function(){
	$('#my_form').validateForm({mode:'tooltip'});
	<?php echo $scripts; ?>
});
